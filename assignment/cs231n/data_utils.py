...
def load_plankton_dataset(path, dtype=np.float32):
  """
  ...
  - y_test: (N_test,) array of test labels; if test labels are not available
    (such as in student code) then y_test will be None.
  """
  # First load class_list
  with open(os.path.join(path, 'resized_train', 'class_list.txt'), 'r') as f:
    class_names = [x.strip() for x in f]

  # Map class to integer labels
  ...
  for i, class_name in enumerate(class_names):
    ...
    # To figure out the filenames we need to open the boxes file
    boxes_file = os.path.join(path, 'resized_train', class_name, 'file_list.txt')
    with open(boxes_file, 'r') as f:
  ...
  # Students won't have test labels, so we need to iterate over files in the
  # images directory.
  img_files = os.listdir(os.path.join(path, 'resized_test'))
  X_test = np.zeros((len(img_files), 3, 48, 48), dtype=dtype)
  for i, img_file in enumerate(img_files):
    img_file = os.path.join(path, 'resized_test', img_file)
    img = imread(img_file)
    if img.ndim == 2:
      img.shape = (48, 48, 1)
    X_test[i] = img.transpose(2, 0, 1)
